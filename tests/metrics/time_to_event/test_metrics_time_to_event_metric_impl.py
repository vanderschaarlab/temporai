...
from tempor.metrics.time_to_event import _metric_impl


def test_validation_check_y_survival():
    with pytest.raises(ValueError, match=".*y must be.*"):
        _metric_impl.check_y_survival(y_or_event="wrong")  # type: ignore

    with pytest.raises(ValueError, match=".*boolean.*"):
        _metric_impl.check_y_survival(
...
def test_validation_check_estimate_1d():
    with pytest.raises(ValueError, match=".*Expected 1D.*"):
        _metric_impl._check_estimate_1d([[1, 1], [2, 2]], test_time=100)  # type: ignore


def test_validation_check_times():
    with pytest.raises(ValueError, match=".*within.*"):
        _metric_impl._check_times(np.asarray([1000]), [1, 4])  # type: ignore


def test_validation_check_estimate_2d():
    with pytest.raises(ValueError, match=".*estimate with.*"):
        _metric_impl._check_estimate_2d(np.asarray([[1, 2], [1, 2]]), np.asarray([1, 3]), np.asarray([1]), Mock())


def test_compute_counts():
    out = _metric_impl._compute_counts(np.asarray([True, True, False]), np.asarray([1, 8, 9]))
    assert len(out) == 4


def test_compute_counts_order_given():
    out = _metric_impl._compute_counts(
        np.asarray([True, True, False]), np.asarray([1, 8, 9]), order=np.asarray([2, 1, 0])
    )
    assert len(out) == 4
...
def test_kaplan_meier_estimator_time_enter():
    ...
    with pytest.raises(ValueError, match=".*left.*"):
        _metric_impl.kaplan_meier_estimator(
            event=np.asarray([True, True, False]),
            time_exit=np.asarray([1, 8, 9]),
            time_enter=np.asarray([0, 1, 0]),
            reverse=True,
...
def test_survival_function_estimator_fit():
    model = _metric_impl.SurvivalFunctionEstimator()
    model.fit(
        y=np.asarray(
...
def test_censoring_distribution_estimator_fit_all():
    model = _metric_impl.CensoringDistributionEstimator()
    model.fit(
        y=np.asarray(
...
def test_estimate_concordance_index_invalid_comparable(monkeypatch):
    monkeypatch.setattr(_metric_impl, "_get_comparable", Mock(return_value=([], [])))

    with pytest.raises(ValueError, match=".*pairs.*"):
        _metric_impl._estimate_concordance_index(Mock(), Mock(), Mock(), Mock())


def test_create_structured_array_validation():
    with pytest.raises(ValueError, match=".*different.*"):
        _metric_impl.create_structured_array([], [], name_event="a", name_time="a")


def test_create_structured_array_non_bool_event():
    y = _metric_impl.create_structured_array([1, 1, 0, 1], [22, 34, 22, 134])
    assert len(y) == 4


def test_create_structured_array_non_bool_event_validation():
    with pytest.raises(ValueError, match=".*binary.*"):
        _metric_impl.create_structured_array([1, 22, 1111, 44544], [22, 34, 22, 134])
    with pytest.raises(ValueError, match=".*0 and 1.*"):
        _metric_impl.create_structured_array([3, 4, 3, 4], [22, 34, 22, 134])


def test_brier_score_reshape():
    out = _metric_impl.brier_score(
...
def test_concordance_index_ipcw_tau_none():
    out = _metric_impl.concordance_index_ipcw(
        survival_train=np.asarray(
            list(zip([True, True, True, True], [1, 2, 11, 4])),
            dtype={"names": ("e", "t"), "formats": ("bool", "f8")},
        ),
