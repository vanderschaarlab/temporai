...
def from_api() -> BaseTreatments:
    return plugin_loader.get("treatments.crn_classifier", random_state=123, epochs=10)
...
def test_CRN_classifier_plugin_sanity(test_plugin: BaseTreatments) -> None:
...
@pytest.mark.parametrize("test_plugin", [from_api(), from_module()])
def test_CRN_classifier_plugin_fit(test_plugin: BaseTreatments) -> None:
    data = get_dummy_data()
...
@pytest.mark.parametrize("test_plugin", [from_api(), from_module()])
def test_crn_classifier_plugin_predict_counterfactuals(test_plugin: BaseTreatments) -> None:
    data = get_dummy_data(temporal_targets_n_features=3)
    test_plugin.fit(data)
