...
from tempor.methods.time_to_event.plugin_ddh import DynamicDeepHitTimeToEventAnalysis
...
def test_sanity(get_test_plugin: Callable, plugin_from: str) -> None:
    ...
    assert test_plugin.full_name() == "time_to_event.dynamic_deephit"
    assert len(test_plugin.hyperparameter_space()) == 10


@pytest.mark.filterwarnings("ignore:.*Validation.*small.*:RuntimeWarning")  # Expected for small test datasets.
@pytest.mark.parametrize("plugin_from", PLUGIN_FROM_OPTIONS)
